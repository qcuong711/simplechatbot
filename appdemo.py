...
def chatbox():
    ...
    def callUpdater(arg=None):
        global name
        global text
        text = recordAudio()
        chat.insert('','end', value=(name+':','    '+wrap(text),'',''), tags='odd')
        threading_s(reply)
    def reply(arg=None):  
        time.sleep(1)
        global text
        if 'who is' in text.lower():
          output = wiki.summary(text[6:len(text)], sentences=2)
          chat.insert('','end', value=('','','Answer:','    '+wrap(output)), tags='even')
        else:
          output= predict(text)
          chat.insert('','end', value=('','','Answer:','    '+wrap(output)), tags='even')
        chat.yview_moveto(1)
    ...
    def greeting(arg=None):
        global name
        name = Entry1.get()
        Entry1.delete(0, 'end')
        chat.insert('','end', value=('User'+':','    '+wrap(name),'',''), tags='odd')
        strt='Hi '+ name+', How can I help you!'
        chat.insert('','end', value=('','','Chatbot:','    '+wrap(strt)), tags='even')
        b1.config(command=lambda: threading_s(callUpdater))
        Entry1.bind("<Return>", callUpdater)
    def greeting1():
        time.sleep(1.5)
        chat.insert('','end', value=('','','Chatbot:','    '+wrap('Hi there! I am chatbot created by Coconut team. First, please tell me your name.')), tags='even')
    global cbwindow

    cbwindow = tk.Tk()
    cbwindow.title("Chatbox")
    cbwindow.geometry("900x700+420+180")
    menuBar = Menu(cbwindow)
    cbwindow.config(menu = menuBar)
    fileMenu = Menu(menuBar)
    fileMenu.add_command(label="About", command=about)
    fileMenu.add_command(label="Exit", command=exit)
    menuBar.add_cascade(label="Option", menu=fileMenu)
    scrollbar = Scrollbar(cbwindow)
    scrollbar.pack( side = RIGHT, fill = Y )
    style = ttk.Style(cbwindow)
    style.theme_use('alt')
    style.configure("Treeview.Heading", font=("Arial", 20, "bold"))
    style.configure("Treeview", rowheight=60, font=("Arial", 11, "italic"),fieldbackground="#e6ffff")
    chat = ttk.Treeview(cbwindow, columns=('name','input', 'answer','output'),selectmode="extended",show="tree",yscrollcommand = scrollbar.set)
    chat['show'] = 'headings'
    chat.column('name', width=60,anchor='n')
    chat.column('input',anchor='nw')
    chat.heading('input', text='INPUT')
    chat.column('answer', width=60,anchor='n')
    chat.column('output',anchor='nw')
    chat.heading('output', text='OUTPUT')
    chat.pack(expand = YES, fill=BOTH)
    chat.tag_configure('odd', foreground="#e60000", background='#e6ffff')
    chat.tag_configure('even', foreground='#002080',background='#e6ffff')
    threading_s(greeting1)
    fr1=tk.Frame(cbwindow)
    fr1.pack(side=BOTTOM)

    Entry1 = tk.Entry(fr1, bd=0, width=40)
    Entry1.pack(pady=5, side = LEFT)
    Entry1.focus()
    Entry1.bind("<Return>", greeting)

    b1 = tk.Button(fr1, text="Submit", command=lambda: threading_s(greeting))
    b1.pack(padx = 5, pady=5, side = LEFT)

    cbwindow.mainloop()

def about():
    mbox.showinfo("About", "Design by coconut team - version 1.0")
